...
class TenantMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        # Let CORS preflight through without auth/tenant checks.
        # Browsers do not send Authorization on OPTIONS preflight.
        ...
        # subdomain only if exists (pn.localhost → pn), normalized to lowercase
        subdomain = parts[0].lower() if len(parts) > 1 else None
